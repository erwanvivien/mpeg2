...
static int simple_tvid_log = 0;
FILE* log_file = NULL;

void dump_state (FILE * f, mpeg2_state_t state, const mpeg2_info_t * info,
		 int offset, int verbose, int simple_tvid_log);
...
static void handle_args (int argc, char ** argv)
{
    ...
    drivers = vo_drivers ();
    while ((c = getopt (argc, argv, "hs::t:pco:vbl::")) != -1)
	switch (c) {
	case 'o':
...
static void decode_mpeg2 (uint8_t * current, uint8_t * end)
{
	...
	if (verbose)
	    dump_state (log_file, state, info,
			total_offset - mpeg2_getpos (mpeg2dec), verbose, simple_tvid_log);
	switch (state) {
	case STATE_BUFFER:
	    return;
	case STATE_SEQUENCE:
